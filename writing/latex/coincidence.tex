...
We fix \(p\) in order to define the equivalences and say that \(p\) is the
\emph{dominating pattern}.

We first define some operations on mesh patterns.
\begin{definition}
Given a mesh pattern \(p\), let \(\addp[D]{p}{(a,b)}\) be the operation that returns
a mesh pattern equivalent to placing a point in the center of box \((a,b)\), where \((a,b)\) is not the
mesh set of \(p\) in
the figure of \(p\), with shading defined by \(D\subseteq\{N,E,S,W\}\).
...
\end{equation*}
\end{example}
Formally, given a mesh pattern \(p = (\pi,R)\) \(\addp[D]{p}{(a,b)}\) gives a mesh
pattern \(p^\prime = (\pi^\prime,R^\prime)\) defined by
\begin{equation*}
...
If the shading set \(D\) is non-empty we can modify definition of the directions
slightly
\begin{equation*}
    \begin{aligned}
        N &= \{(a,b+1),(a+1,b+1)\} \\
...
\begin{example}
    ...
    \end{align*}
\end{example}
We can define these formally as with \(\addp[D]{p}{(a,b)}\). Considering
only adding the ascent, adding a descent is very similar
\begin{equation*}
    \addi{p}{(a,b)} = (\pi^\prime, R^\prime)
\end{equation*}
\begin{equation*}
    \pi^\prime(i) = \begin{cases}
    \pi(i) & \text{if } i\neq a+1,a+2 \text{ and } \pi(i)<b\\
    \pi(i)+2 & \text{if } i\neq a+1,a+2 \text{ and } \pi(i)>b\\
    b+1, b+2 & \text{if } i = a+1, a+2
\end{cases}
\end{equation*}
and
\begin{equation*}
        R^\prime = \bigcup_{(i,j)\in R}r((i,j)) \cup 
\end{equation*}
Where \(r((i,j))\) is defined by
\begin{equation*}
    r((i,j)) =
...
data is calculated on permutations up to length 11.
...
Through experimentation we discover that there are at most 29 coincidence
...
explained by repeated application of \PropositionRef{prop:dom1}.
...
Consider an occurrence of \(m_1\) in a permutation in \(\av{\perm{2,3,1}}\), then if the region corresponding
...
must therefore contain a decreasing subsequence. This gives rise to the following lemma:
...
\begin{equation*}
    ...
    \end{tikzpicture}
\end{equation*}
We know that the region corresponding to the box \((1,1)\) contains a decreasing subsequence. The top point
of this decreasing subsequence can be chosen to act as the second point in the mesh pattern,
...
choosing the two leftmost points give us the following mesh pattern.
\begin{equation*}
