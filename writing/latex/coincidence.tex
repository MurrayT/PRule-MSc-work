...
\begin{definition}
    ...
    as the operations that return a mesh pattern equivalent to placing a
...
and
\begin{equation*}
        R^\prime = \bigcup_{(i,j)\in R}r((i,j)) \cup \text{crap to deal with strong ascent},
\end{equation*}
where \(r((i,j))\) is defined by
\begin{equation*}
    r((i,j)) =
...
are at most 29 coincidence classes of mesh patterns with underlying classical
pattern \(\perm{2,1}\).
...
explained by application of \PropositionRef{prop:dom1}. % repeated?
...
classes. Experimentation shows that there are in fact at most \(39\) coincidence
...
If we let \(z\) be the topmost point in this decereasing subsequence, then \(xz\)
...
\begin{proof}
    ...
    If the region is empty, the occurrence of \(m_1\) is trivially an occurence
    of \(m_2\).
