...
\eqref{eqn:genL1} and \eqref{eqn:genL2} are equal, this occurs if and only if
\(C(x)(G(x)-1) = (C(x)-1)^2\).
\begin{align*}
        (C(x)-1)^2 &= C(x)(G(x)-1)\\
        \shortintertext{Using the definition of \(G\) and \(C\)}
        x^2C^4(x) &=  (1+xC(x))(xC^2(x)-xC(x))\\
        xC^3(x)&=C(x)-1+xC^3(x)-xC^2(x)\\
        0 &= C(x) - 1 -xC^2(x)\\
        C(x) &= 1 +xC^2(x)
\end{align*}
The final line is always satisfied, and therefore the two generating functions
are equal.
...
The second pattern occurs if the last element is higher than
the penultimate element. This can only occur if the last element is
in a single block in the set partition context, In order to construct
a avoider of length \(n\) we can take any avoider \(\perm{2,3,1}\)
of length \(n-1\) and insert the new maximum into the last block.
This ensures that the last block is never a singleton. This means
...
\end{equation*}
Let \(\mathcal{N}_1\) be the set of avoiders of \(p_1\), by structural decomposition
around the leftmost point we have
\begin{equation*}
    \mathcal{N}_1 = \varepsilon \sqcup
    \decompleft{0/1,1/0}{\mathcal{N}_1}{}{}{\mathcal{N}^\prime_1}
\end{equation*}
Where \(\mathcal{N}^\prime_1\) is a permutation avoiding \(\perm{2,3,1}, m_1\)
and \(\textpattern{}{1}{0/0,0/1,1/1}\)
Now consider the decomposition of a permutation in \(\mathcal{N}^\prime_1\) it can
once again be decomposed around the leftmost point
\begin{equation*}
    \mathcal{N}^\prime_1 = \varepsilon \sqcup
    \decompleft{0/1,1/0}{\scriptstyle \mathcal{N}_1\setminus\varepsilon}{}{}{\mathcal{N}^\prime_1}
\end{equation*}
This is a complete decomposition of avoiders of \(p_1\). Now we
look at an avoider of \(p_2\), this time decomposition is around the maximum
\begin{equation*}
    \mathcal{N}_2 = \varepsilon \sqcup
    \decompleft{0/1,1/0}{\mathcal{N}_2}{}{}{\mathcal{N}^\prime_2}
\end{equation*}

Again we use the same method of decomposition of a permutation in \(\mathcal{N}^\prime_2\)
\begin{equation*}
    \mathcal{N}^\prime_2 = \varepsilon \sqcup
    \decompleft{0/1,1/0}{\scriptstyle \mathcal{N}_2\setminus\varepsilon}{}{}{\mathcal{N}^\prime_2}
\end{equation*}

This gives us a generating function \(N(x)\) satisfying
\begin{align}
    N(x) &= 1 + xN(x)N^\prime(x) \label{eqn:Ngen}\\
    N(x) &= 1 + x(N(x)-1)N^\prime(x)\label{eqn:NprimeGen}
\end{align}

Solving \EquationRef{eqn:NprimeGen} for \(N^\prime(x)\) and substituting into
\EquationRef{eqn:Ngen} gives us the fact that a the generating function for
\(N(x)\) satisfies
\begin{equation}
    N(x) = xN^2(x) - x(N(x) - 1) + 1
\end{equation}
