...
and \(p\) is a mesh pattern of length 2 and establish sufficient conditions for two such
...
\end{equation*}
We allow composition of these modifications. % TODO Are they ever used?
...
% TODO There is some inconsistency in when you put the set brackets into Av
...
\begin{proof}[Proof of \PropositionRef{prop:dom1}]
    ...
    points in this region, which implies this is an occurrence of $m_2$. Hence every occurrence of
...
\begin{proof}
    ...
    implies and occurrence of \(m_2\).
    We consider taking the first branch of every choice.
    ...
    condition~\ref{prop:dom2:condc} of the proposition it must contain
...
By applying both the First Dominating Pattern Rule and the Second Dominating
Pattern Rule fully explains coincidences of classes of the form
...
the case when this region is not empty, adn consider choosing the rightmost
point in the region.
...
\begin{description}
    ...
    \item [Inductive Step \((n=k+1)\)] Suppose that we have \((k+1)\) points in
        the middle box. Choose the bottom most point in the middle box, which gives the
        mesh pattern
     \begin{equation*}
