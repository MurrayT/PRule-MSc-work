...
\FigureRef{fig:rule1} we can gain shading in the boxes \(\boks{0,2}\), \(\boks{2,0}\)
...
\begin{proof}[Proof of \PropositionRef{prop:dom1}]
    Since \(R_1\) is a subset of \(R_2\), \NoteRef{not:downcmesh} implies that
...
classical pattern is \(21\). By \NoteRef{note:main} there are
...
\(56\) \(\coinc[(3)]{231}\)-equivalence classes, of mesh patterns where the underlying
classical pattern is \(12\). By \NoteRef{note:main} there are
therefore exactly \(56\) \(\coinc{231}\)-equivalence classes, since we have
observed \(56\) \(\coinc[comp]{231}\)-equivalence classes through experimentation.
...
that there are \(7\) \(\coinc[comp]{321}\)-equivalence classes are unexplained.
Since the number of coincidences is
...
Consider the region corresponding to the box \(\boks{0,1}\) in any occurrence of \(m_1\)
...
If there is only one point in the region we can choose this point to replace the \(1\) in
...
\begin{equation*}
    \pattern{3,4}{2,3,1,4}{0/0, 0/1, 0/2, 0/3, 0/4,
                            ...
                            4/1, 4/2, 4/3}
\end{equation*}
Here the two highlighted points are the original two points.
The other two points are an occurrence of the pattern
we \(m_2\), and hence the two patterns are coincident.
...
Because of symmetries we have dealt with \(4\) out of the \(7\) \(\coinc[comp]{321}\)-equivalence classes.
...
\begin{description}
    ...
    \end{equation*}
        ...
        as any points in the boxes \(\boks{1,2}\) and \(\boks{1,3}\) would create an occurrence of the dominating pattern \(\perm{3,2,1}\)
     \begin{equation*}
        \begin{tikzpicture}[scale=0.5]
...
total in \(\av{\{\perm{3,2,1}, \mperm{1,2}{R}\}}\) there are \(\coinc{321}\)-equivalence
classes.
