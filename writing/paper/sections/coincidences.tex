...
\ie, \(\av{\lambda} = \av{\gamma}\). This is equivalent to \(\lambad\) and \(\gamma\) being contained in
...
\(\av{\pi,p_1}\) and \(\av{\pi,p_2}\) are equal.
...
\(\coinc[comp]{\pi}\)-coincidence classes.

If \(p_1 \coinc{\pi} p_2\), it can be seen that
\(p_1 \coinc[comp]{\pi} p_2\), and therefore the \(\coinc{\pi}\)-equivalence
classes form partitions of the \(\coinc[comp]{\pi}\)-coincidence classes.
...
a combination of all three rules shows coincidences. These three equivalence
relations are successive refinements of \(\coinc{\pi}\). Therefore if we find
that \eg, \(\coinc[(1)]{\pi} = \coinc[comp]{\pi}\) then \(\coinc[(1)]{\pi} = \coinc{\pi}\).
