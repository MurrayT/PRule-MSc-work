...
the \emph{dominating pattern}. Given such a dominating pattern \(\pi\) we will write \(q_1 \coinc{\pi} q_2\) if for the mesh patterns, \(q_1\) and \(q_2\),  the sets \(\av{p,q_1}\) and \(\av{p,q_2}\) are coincident.

Our first step is to calculate whether \(\av{p,q_1}\) and \(\av{p,q_2}\) are equal up to permutations of length 11 computationally, if they are we write \(q_1 \coinc[comp]{\pi} q_2\). If \(q_1 \coinc{\pi} q_2\), it can be seen that \(q_1 \coinc[comp]{\pi} q_2\), and therefore the \(\coinc{\pi}\)-equivalence classes form partitions of the \(\coinc[comp]{\pi}\)-coincidence classes.

We then form three Propositions called ``Dominating Pattern Rules'', if the First Dominating
Pattern Rule shows that \(q_1 \coinc{\pi} q_2\) then we write \(q_1 \coinc[(1)]{\pi} q_2\).
If a combination of the First and Second Dominating Pattern Rules show that \(\av{p,q_1}= \av{p,q_2}\) then we write \(q_1 \coinc[(2)]{\pi} q_2\); similarly we write \(q_1 \coinc[(3)]{\pi} q_2\) if a combination of all three rules shows coincidences.
...
that there are at least \(29\) \(\coinc[comp]{321}\)-coincidence classes where the underlying classical pattern of the mesh pattern is \(21\).
...
\begin{proposition}[First Dominating Pattern Rule]
    ...
    as a consequence of the first dominating pattern rule if
    \begin{enumerate}
...
\begin{proof}[Proof of \PropositionRef{prop:dom1}]
    We need to prove that \(m_1 \coinc{\pi} m_2\).
...
By application of \PropositionRef{prop:dom1} we obtain \(43\) \(\coinc[(1)]{231}\)-coincidence
...
classes between mesh patterns with \(21\) as an underlying classical pattern, for example the following two patterns seem to be coincident in
...
point in the mesh pattern. \FigureRef{fig:d2} show how the cases apply to a general container of \(m_1\) to transform it into a container of \(m_2\) , the circled points are the same points in the permutation.
\begin{figure}
\begin{tikzpicture}
	\modpattern[3]{1,2}{2,1}{0/0,0/1,0/2,1/0,2/1}
	\filldraw (1.33,1.66) circle (1.5pt);
	\filldraw (1.66,1.33) circle (1.5pt);
	\filldraw[pattern=crosshatch dots, pattern color=black!80] (2.1,0.9) rectangle (2.5,0.0);
	\filldraw[pattern=crosshatch dots, pattern color=black!80] (2.5,3) rectangle (2.9 ,2.1);
\end{tikzpicture}
\raisebox{8ex}{\(\coinc{}\)}
\begin{tikzpicture}
	\modpattern[3]{1}{2,1}{0/0,0/1,0/2,1/0,1/1,2/1}
	\filldraw (2.15,0.85) circle (1.5pt);
	\filldraw (2.3,0.7) circle (1.5pt);
	\draw (2.3,0.7) circle (3pt);
	\filldraw[pattern=crosshatch dots, pattern color=black!80] (2.4,0.6) rectangle (2.65,0.0);
	\filldraw[pattern=crosshatch dots, pattern color=black!80] (2.65,3) rectangle (2.9 ,2.1);
\end{tikzpicture}
