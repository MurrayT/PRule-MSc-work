...
In order to describe the rules it is useful to have a notion for inserting
...
defined in \DefinitionRef{def:ap}. We define the following four modifications of
\(p^\boks{i,j}\).
\begin{align*}
...
\end{align*}
\end{definition}
Informally, these are considering the topmost, rightmost, leftmost, or
...
We now attempt to fully classify coincidences in families characterised by
...
underlying classical pattern is \(21\). By \NoteRef{note:main} there are
...
\end{proposition}
\begin{proof}
    By \NoteRef{not:downcmesh} we only need to show that an occurrence of
...
the original point in the mesh pattern. \FigureRef{fig:d2} show how the cases
...
\end{proposition}
\begin{proof}
    \NoteRef{not:downcmesh} implies \(\av{\{\pi,m_1\}} \subseteq \av{\{\pi,m_2\}}\)
...
underlying classical pattern is \(12\). By \NoteRef{note:main} there are
...
\(m_1\) in a permutation. By \LemmaRef{lem:incdec} it must contain an increasing
