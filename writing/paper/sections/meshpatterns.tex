...
position of the occurrence of \(\tau\) in \(\pi\). These restrictions say that
no elements of \(\pi\) are allowed in the regions of the plot corresponding
to shaded boxes in the mesh. These boxes are denoted by \(\boks{i,j}\), where the point \((i,j)\) is
...
\(\pi, G(\pi) = \{(i,\pi(i)\mid i\in[1,n]\}\) such that there are order-preserving
...
\end{enumerate}
We call \(R_{ij}\) the region corresponding to \(\boks{i,j}\).
...
contained in the mesh set of \(q\), in this case we call \(p\) a \emph{subpattern}
of \(q\).
\begin{definition}
A mesh pattern \(q=(\kappa,T)\) \emph{contains} a mesh pattern \(p=(\tau,R)\) as a \emph{subpattern}
if \(\kappa\) contains \(\tau\) and \(\left(\bigcup_{\boks{i,j}\in R} R_{ij}\right)\subseteq T\).
\end{definition}
