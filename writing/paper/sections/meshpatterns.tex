...
\(\sigma = \perm{2,3,1}\) can be seen in \FigureRef{fig:plots}.
\FigureRef{fig:containment} shows the containment of \(\sigma\) in \(\pi\) as in
\ExampleRef{ex:contexmpl}.

The boxes in the plot of a permutation are denoted by \(\boks{i,j}\), where the
